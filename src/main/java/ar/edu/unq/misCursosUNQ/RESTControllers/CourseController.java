...
public class CourseController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<Course> getCourseById(@PathVariable("id") Integer id) throws RecordNotFoundException {
        Course entity = service.getCourseById(id);
        return new ResponseEntity<Course>(entity, new HttpHeaders(), HttpStatus.OK);
    }
 
    @PostMapping
    public ResponseEntity<Course> createOrUpdateCourse(Course course) throws RecordNotFoundException {
        Course updated = service.createOrUpdateCourse(course);
        return new ResponseEntity<Course>(updated, new HttpHeaders(), HttpStatus.OK);
    }
 
    @DeleteMapping("/{id}")
    public HttpStatus deleteCourseById(@PathVariable("id") Integer id) throws RecordNotFoundException {
